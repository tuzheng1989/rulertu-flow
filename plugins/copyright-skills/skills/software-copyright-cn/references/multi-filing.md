...
3. **代码行达标**：每份代码行（strip_source 代码行口径）≥ `--min-code-lines`（默认 3,000，用户口径：每份必须满 3,000 代码行，对齐官方"不足 3,000 行交全部"的分档阈值，每份按满额规格独立成材料）→ 低于即阻断。模块切分时就要保证每份凑足 3,000 行自有代码；凑不足的模块不单独申报。
