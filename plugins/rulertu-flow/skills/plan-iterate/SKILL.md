...
对方案最多评审五轮。每轮使用共享 [review schema](scripts/review-schema.json)；`score >= 8.5` 且没有 P0/P1 才达标。中间产物位于 `<plan目录>/.plan-iterate/<plan名>/`：`review-RN.json`、`review-RN.log`、`state.json`；外部后端另有 `review-RN.events.jsonl` 与 `review-RN.stderr.log`。
...
两个外部后端都只在用户明确授权且目标 CLI 可用时运行；可用性以首轮评审调用本身作为冒烟，失败按进程失败处理，不回退为同宿主自评。后端切换或原会话无法恢复时，新建目标评审会话并注入上一轮 JSON。
...
4. 使用同一 reviewer/session 复审当前磁盘内容。最多五轮，无论是否达标都输出逐轮评分、遗留问题和产物路径。
