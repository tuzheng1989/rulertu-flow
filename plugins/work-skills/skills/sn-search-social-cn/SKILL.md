...
```bash
python -m pip install -r skills/sn-search-social-cn/requirements.txt
```
...
```bash
python scripts/bilibili_search.py <query> [选项]
```
...
```bash
python scripts/bilibili_search.py "机器学习教程" --limit 5
python scripts/bilibili_search.py "Python" --order click --limit 10
```

### zhihu_search.py

```bash
python scripts/zhihu_search.py <query> [选项]
```
...
```bash
ZHIHU_COOKIE="..." python scripts/zhihu_search.py "Python 异步编程" --limit 5
python scripts/zhihu_search.py "大模型" --cookie "..." --type topic --limit 5
```

### douyin_search.py

```bash
python scripts/douyin_search.py <query> [选项]
```
...
```bash
DOUYIN_COOKIE="..." python scripts/douyin_search.py "编程教程" --limit 5
```
