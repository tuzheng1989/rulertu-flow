...
for chunk in agent.stream(
    ...
    if chunk["type"] == "updates":
        ...
        if is_subagent:
            # Extract tool call ID from namespace
            tool_call_id = next(
...
When subagents use tools, you can stream tool call events to display what each subagent is doing. Tool call chunks appear in `messages` stream mode.
...
for chunk in agent.stream(
    ...
    if chunk["type"] == "messages":
        ...
        # Identify source: "main" or subagent namespace segment
        is_subagent = any(s.startswith("tools:") for s in chunk["ns"])
...
def analyze_data(topic: str) -> str:
    ...
    This tool performs actual analysis and emits progress updates.
    ...
    if chunk["type"] == "updates":
        for node_name, data in chunk["data"].items():
            ...
            if chunk["ns"] and chunk["ns"][0].startswith("tools:"):
                ...
                # Note: pregel task ID differs from the tool_call_id,
            ...
            # that subagent has completed and returned its result.
...
The v2 format eliminates nested tuple unpacking, making it straightforward to handle subgraph streaming in Deep Agents.

For more details on the v2 format, including type narrowing and Pydantic/dataclass coercion, see the LangGraph streaming docs.

---

**See also:**

- [[subagents.md]] — Configure and use subagents with Deep Agents
- Frontend streaming — Build React UIs with `useStream` for Deep Agents
- LangChain streaming overview — General streaming concepts with LangChain agents
